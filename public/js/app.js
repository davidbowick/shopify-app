...
function checkSelects(me) {
  ...
  var $this = me,
      ...
      opt0 = [],
      opt1 = [],
      opt2 = []; // if($this.hasClass('option-0')

  var myIndex = $this.attr('class').split('-')[1]; // console.log(myIndex);
  ...
  select.find('option').each(function (i, v) {
    ...
    if (myText.indexOf(selectedText) > -1) {
      ...
      $.each(vSplit, function (vi, vo) {
        // 1st one changed
        var viVal = vSplit[vi].replaceAll('"', '˝');
  ...
  }).prop('selected', true); // console.log(mySelectedValues);
...
$(function () {
      ...
      $.ajax({
        ...
        success: function success(d) {
          ...
          if (d.length) {
            ...
            showHide();
            $('.main-preloader').fadeOut(); // console.log(d);

            $('.no-results').hide(); // 	$(d).each(function(i,p) {
  ...
  $(document).on('blur', '.money-input', function () {
    ...
    $(this).val(valToFixed); //$(this).val(formatCurrency($(this).val().replace(/[,$]/g,'')));
  });
  ...
  $('.money-input').blur(); // $(document).on('blur')
  ...
  $(document).on('click', '.custom--create-btn', function (e) {
    ...
    var extendedPrice = $('#custom-extended-price').val(); // var priceCurrency = '$'+price+'.00';
    ...
    var priceCurrency = formatter.format(extendedPrice);
    /* $2,500.00 */
    ...
    squareFootage ? obj.attr('data-square-footage', squareFootage) : ''; //basePrice ? obj.attr('data-base-price',basePrice) : '';

    var title = $('<div class="draft-order__title"><h5>Custom Product</h5></div>').appendTo(obj); // var properties = $('<div class="draft-order__properties"></div>').appendTo(obj);

    title.append('<span class="draft-order__variant">' + size + ' / ' + $info.join(' / ') + '</span>'); // var priceWeight = $('<div class="draft-order__priceWeight"></div>').appendTo(obj);
    // title.append('<br>' + priceCurrency + ' / ' + weight + 'lbs');

    var inputs = $('<div class="flex flex--wrap grid--half-gutters"></div>').appendTo(obj);
    var qty = $('<div class="grid__item field inline--label"><label>Qty</label><input class="variant-quantity" name="quantity[]" type="number" value="' + qty + '"></div>').appendTo(inputs); //var hiddenInput = $('<input type="hidden" value="'+$id+'" class="variant-id" name="variant_id[]">').appendTo(inputs);
  ...
  $(document).on('click', '.btn--download-csv', function (e) {
    e.preventDefault(); // var draftOrderName = $('#gotoDraftOrder .btn').data('name')
    ...
    $('.draft-order__custom-product').each(function (i, v) {
      ...
      $.each(customFormElements, function (c, v) {
        var slug = slugify(v);
        var isLastElement = c == customFormElements.length - 1; // $this.data(slug)
  ...
  $('#create-draft-order').submit(function (e) {
    ...
    $.ajax({
      ...
      success: function success(data) {
        ...
        $(createDraftOrderBtn).find('svg').hide(); // console.log(data);
  ...
  $(document).on('click', '.btn--start-over', function (e) {
    ...
    $('.custom--square-footage').html('&nbsp;'); // $(this).closest('.draft-order__saved-product').remove();
  ...
  $(document).on('click', '.calculate--square-footage', function (e) {
    ...
    if (w > 0 && h > 0) {
      ...
      squareFootage = Math.round(squareFootage * 100) / 100; //$(this).hide();
        ...
        success: function success(d) {
          ...
          if (d.length) {
            ...
            $(d).each(function (i, p) {
              ...
              var email = $('<div class="user-list__name">' + $email + '</div>').appendTo(datawrapper); // var defaultAddress
...
__webpack_require__(/*! /Users/davidbowick/code/shopifyapp/resources/js/app.js */"./resources/js/app.js");
module.exports = __webpack_require__(/*! /Users/davidbowick/code/shopifyapp/resources/sass/app.scss */"./resources/sass/app.scss");
